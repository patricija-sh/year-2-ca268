...
class BST:
    ...
    def is_present(self, target):
        return self.rec_is_present(self.root, target)
    
    def rec_is_present(self, root, target):
        if root is None:
            return False
        elif root.item == target:
            return True
        else:
            return self.rec_is_present(root.left, target) or self.rec_is_present(root.right, target)
