...
class BST:
    ...
    def count(self, lo, hi):
        return self.range_rec_count(self.root, lo , hi)
        
    def range_rec_count(self, root, lo, hi):
        if root is None:
            return 0
        elif root.item == lo and root.item == hi:
            return 1
        elif root.item >= lo and root.item <= hi:
            return 1 + self.range_rec_count(root.left, lo, hi) + self.range_rec_count(root.right, lo , hi)
        elif root.item < lo:
            return self.range_rec_count(root.right, lo, hi)
        else:
            return self.range_rec_count(root.left, lo, hi)
